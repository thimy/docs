class Builders::Sidebar < SiteBuilder
  def build
    generator do
      ...
      site.data.sidebars.guides.each do |specification_name, datalist|
        datalist.each do |version_name, attributes|
  ...
  def build_resources(datalist, collection, root)
    if datalist.present?
      datalist.map do |list_item|
